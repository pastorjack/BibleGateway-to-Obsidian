...
translation='LSB'    # Which translation to use
...
# filename=${export_prefix}$chapter # Setting the filename
  ...
  text=$(echo "$text" | sed 's/^(.*?)1/1/') # Deleting unwanted headers
...
yaml_start="---\n"
yaml_end="\n---\n"
alias="Aliases: [${book} ${chapter}]\ncssclass: bible" # Add other aliases or 'Tags:' here if desired. Make sure to follow proper YAML format.
...
find . -name '*.md' -type f -exec sed -i '/^##### Chapter/d' {} +
find . -name '*.md' -type f -exec sed -i '/Legacy Standard/d' {} +
